...
function baseBallPointer(values) {
  let temp_array = [];
  for (var i = 0; i < values.length; i++) {
    ...
    if (values[i] == "C") {
      ...
      temp_array.pop();
      var score = temp_array[temp_array.length - 1];
      temp_array.push(score * 2);
    } else if (values[i] == "+") {
      temp_array.pop();
      temp_array.push(
        parseInt(temp_array[temp_array.length - 1]) +
          parseInt(temp_array[temp_array.length - 2])
      );
    }
  }
  let sum = 0;
  for (var i = 0; i < temp_array.length; i++) {
    sum += parseInt(temp_array[i]);
  }
  return sum;
}
// TODO: write your code above this line
